...
export default function Home() {
  ...
  const T: Record<string, Record<string,string>> = {
    ...
    runyoro: { title:'SACCO Wallet', sub:"Obusindi bw'Obuhinzi", login:'Injira', phone:"Ennamba y'Esiimu", password:'Ekigambo Kyobujagwa', register:'Omuhizi Omushasha? Jimu', forgot:'Wibagiire ekigambo?', tagline:'Sindika. Gula. Hinda.', logging:'Onjira...' },
    luganda: { title:'SACCO Wallet', sub:"Ensimbi z'Abalimba", login:'Yingira', phone:"Ennamba y'Simu", password:'Ekigambo Kyobukuumi', register:'Omulimu Omupya? Wandiika', forgot:'Ggwaana ekigambo?', tagline:'Leka. Saba. Grow.', logging:'Onjira...' },
  }
  const txt = T[lang]

  function phoneToEmail(p: string) {
    return p.includes('@') ? p : p.replace(/^0/, '256').replace(/^\+/, '') + '@saccomember.ug'
  }
  ...
  async function handleLogin(e: React.FormEvent) {
    ...
    try {
      ...
      setError('Wrong phone or password. Try again.')
    }
    setLoading(false)
  }
    ...
    <div style={{ minHeight:'100vh', background:'linear-gradient(160deg,#1a6b3a 0%,#0f4225 60%,#0a2e19 100%)', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding:24 }}>
      ...
      <form onSubmit={handleLogin} style={{ width:'100%', maxWidth:360, display:'flex', flexDirection:'column', gap:12 }}>
        ...
        <input type="text" value={phone} onChange={e=>setPhone(e.target.value)} placeholder={txt.phone}
          style={{ width:'100%', border:'2px solid rgba(255,255,255,0.2)', borderRadius:14, padding:'14px 16px', fontSize:16, background:'rgba(255,255,255,0.1)', color:'white', outline:'none' }} required />
        ...
        <Link href="/farmer/forgot-password" style={{ textAlign:'center', color:'rgba(255,255,255,0.5)', fontSize:13, textDecoration:'none' }}>{txt.forgot}</Link>
        <Link href="/admin" style={{ textAlign:'center', color:'rgba(255,255,255,0.3)', fontSize:12, textDecoration:'none', marginTop:8 }}>⚙️ Admin Login</Link>
      </form>
