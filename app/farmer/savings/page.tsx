...
export default function Savings() {
  ...
  const load = useCallback(async () => {
    if (!user?.id) return
    const { data: w } = await supabase.from('wallets').select('id,savings_balance').eq('user_id', user.id).single()
    if (w) setWallet(w)
    const { data: t } = await supabase.from('savings_transactions')
      .select('id,type,amount,note,created_at').eq('farmer_id', user.id)
  ...
  async function handleTxn() {
    if (!wallet || !user?.id || !amount) return
    ...
    const ref = `${user.id}-${Date.now()}`
    const { error: txErr } = await supabase.from('savings_transactions').insert({
      wallet_id: wallet.id, farmer_id: user.id, type: modal === 'deposit' ? 'deposit' : 'withdrawal',
      amount: amt, balance_after: newBal, note: note || null, reference_id: ref,
    })
    if (!txErr) {
      await supabase.from('wallets').update({ savings_balance: newBal, balance: newBal }).eq('id', wallet.id)
      setSuccess(modal === 'deposit' ? `✅ Saved ${fmt(amt)}` : `✅ Withdrew ${fmt(amt)}`)
      setModal(null); setAmount(''); setNote('')
      await load()
      setTimeout(() => setSuccess(''), 4000)
    } else { setError(txErr.message) }
    setLoading(false)
  }
  ...
  return (
    <div style={{ padding:16 }}>
      ...
      <div style={{ background:'linear-gradient(135deg,#1a6b3a,#2d9e56)', borderRadius:20, padding:24, marginBottom:20, color:'white', textAlign:'center' }}>
        ...
        <p style={{ margin:0, fontSize:13, opacity:0.7 }}>Keep saving every month 💪</p>
      </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12, marginBottom:20 }}>
        <button onClick={() => setModal('deposit')} style={{ background:'#1a6b3a', color:'white', border:'none', borderRadius:16, padding:16, fontWeight:700, fontSize:16, cursor:'pointer' }}>
          ⬆️ Deposit
        </button>
        <button onClick={() => setModal('withdraw')} style={{ background:'white', color:'#1a6b3a', border:'2px solid #1a6b3a', borderRadius:16, padding:16, fontWeight:700, fontSize:16, cursor:'pointer' }}>
          ⬇️ Withdraw
        </button>
      </div>
        ...
        <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.5)', zIndex:200, display:'flex', alignItems:'flex-end' }}>
          <div style={{ background:'white', borderRadius:'20px 20px 0 0', padding:24, width:'100%' }}>
            ...
            <button onClick={() => { setModal(null); setError('') }}
              style={{ width:'100%', background:'transparent', color:'#6b7280', border:'none', padding:12, fontSize:16, cursor:'pointer' }}>
              Cancel
            </button>
          </div>
        </div>
