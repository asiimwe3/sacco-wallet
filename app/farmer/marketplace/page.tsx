...
const DEMO: Listing[] = [
  ...
  { id:'d5', seller_name:'AgriService Kyenjojo', seller_village:'Kyenjojo Town', seller_type:'store', title:'Land Ploughing Service', description:'Tractor ploughing. One acre from UGX 120,000.', price:120000, unit:'acre', quantity_available:999, category:'services', phone:'0782500000', whatsapp:'256782500000', posted_date:new Date().toISOString().slice(0,10), is_negotiable:true, delivery_available:true },
...
const CAT_ICONS: Record<string,string> = { crops:'🌾', livestock:'🐄', seeds:'🌱', tools:'🔧', services:'🚜', produce:'🥬' }
...
export default function Marketplace() {
  ...
  useEffect(() => {
    async function load() {
      const snap = await getDocs(query(collection(db, COLLECTIONS.MARKETPLACE), where('is_active','==',true), orderBy('posted_date','desc'), limit(50)))
      if (!snap.empty) setListings([...snap.docs.map(d=>({id:d.id,...d.data()} as Listing)), ...DEMO])
    }
    load()
  }, [])
  ...
  async function handlePost() {
    ...
    setPosting(true)
    await addDoc(collection(db, COLLECTIONS.MARKETPLACE), {
      seller_id: user.uid, seller_name: profile.full_name, seller_village: profile.village||'Kyenjojo',
  ...
  if (selected) return (
    <div style={{ padding:16 }}>
      <button onClick={() => setSelected(null)} style={{ background:'none', border:'none', fontSize:16, cursor:'pointer', color:'#374151', marginBottom:16 }}>← Back</button>
      <div className="card" style={{ marginBottom:12 }}>
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:12 }}>
          <div>
            ...
            {selected.is_negotiable && <span style={{ background:'#dcfce7', color:'#16a34a', borderRadius:999, padding:'2px 8px', fontSize:12, fontWeight:600, display:'inline-block', marginTop:4 }}>Negotiable</span>}
          </div>
      ...
      <a href={`https://wa.me/${selected.whatsapp}?text=Hi, I'm interested in: ${selected.title}`} target="_blank" rel="noopener noreferrer"
  ...
  return (
    <div style={{ padding:16 }}>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:16 }}>
        <div><h2 style={{ margin:0, fontSize:22, fontWeight:800 }}>🏪 Marketplace</h2><p style={{ margin:0, color:'#6b7280', fontSize:13 }}>{filtered.length} listings</p></div>
      ...
      <input type="text" value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search listings, village..."
        style={{ width:'100%', border:'2px solid #e5e7eb', borderRadius:12, padding:'12px 16px', fontSize:16, outline:'none', marginBottom:12 }} />
      ...
      {showSell && (
        <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.5)', zIndex:200, display:'flex', alignItems:'flex-end' }}>
          <div style={{ background:'white', borderRadius:'20px 20px 0 0', padding:24, width:'100%', maxHeight:'85vh', overflowY:'auto' }}>
            ...
            {[{key:'title',label:'Title',type:'text'},{key:'price',label:'Price (UGX)',type:'number'},{key:'unit',label:'Unit (kg, piece...)',type:'text'},{key:'quantity',label:'Quantity Available',type:'number'},{key:'phone',label:'Contact Phone',type:'tel'}].map(f=>(
            ...
            <div style={{ marginBottom:12 }}>
              ...
              <textarea value={form.description} onChange={e=>setForm(p=>({...p,description:e.target.value}))} rows={3}
                style={{ width:'100%', border:'2px solid #e5e7eb', borderRadius:12, padding:'12px 14px', fontSize:16, outline:'none', resize:'none' }} />
            </div>
            <div style={{ marginBottom:12 }}>
